...
locations <-
  tibble(
    location = c("OST", "BLA", "ZEE", "DUI", "KNO", "ZWA", "ZOU"),
...
date_OST <-
  as.numeric(c(max(df_sal$year[df_sal$location == "OST"]),
               min(df_sal$year[df_sal$location == "OST"])))
date_ZEE <-
...
# Print plots to file
pdf("sup_figure_2.pdf", height = 3.8, onefile = F)
ggarrange(
  p1,
  p2,
  ncol = 2,
