...
filelist <- tibble(path = list.files(path = "data/shell/shell_images", 
                                     pattern = "\\.jpg", 
                                     full.names=TRUE)) %>% 
...
nucella_coo <- Out(nucella_xy) 

stack(nucella_coo)

# Import explanatory variables
nucella_coo_fac <- tibble(read_delim("data/shell/sample_list.csv",delim=";"))

# Merge Coo object with explanatory variables
nucella_coo$fac <- nucella_coo_fac 
...
nucella_coo_pr <- nucella_coo %>% 
  ...
  coo_center() %>% 
  # Align pseudo-landmarks
  coo_align() %>% 
  # Scale pseudo-landmarks
  coo_scale() %>%  
  # Sample pseudo-landmarks with 1000 points
  coo_sample(1000) %>% 
...
cal_shape <- calibrate_reconstructions_efourier(coo_smooth(nucella_coo_pr, 1000), 
                                                range = 1:12)
plot(cal_shape)
...
# Harmonic power
cal_harm <- calibrate_harmonicpower_efourier(nucella_coo_pr, nb.h = 11)

# Show harmonic power
cal_harm$minh # 9 harmonics gather 99% of the total harmonic power
...
mo_pan_0 <- plot_grid(title_mo_pan_0, mo_pan_0, ncol=1, rel_heights=c(0.1, 1))

# Elliptic Fourier analysis of shell shapes 
nucella_coe <- efourier(nucella_coo_pr, nb.h = 9, norm=FALSE)
...
# The first 3 PCA describe ~91% and the first 5 PCA describe ~95% of the shape 
...
# Export to file ----------------------------------------------------------
# Export first 5 PCs which cover more than 95% of possible shell shape variance.
nucella_PCs <- data.frame(sample = nucella_pca$fac$sampleID,
                          country = nucella_pca$fac$country,
                          location = nucella_pca$fac$location,
                          year = nucella_pca$fac$year,
                          PC1 = nucella_pca$x[, 1],
                          PC2 = nucella_pca$x[, 2],
                          PC3 = nucella_pca$x[, 3],
                          PC4 = nucella_pca$x[, 4],
...
write.csv(nucella_PCs, "data/shell/shell_outline_PCA.csv", row.names = FALSE)
saveRDS(nucella_coe, "data/shell//nucella_coe_object.rds")
